...
//Поиск чисел в чар-массиве
void searchDigits_v2(char* array,int* array_int,int size_array,int* _array_int_index)
{
    //char digits[2];
    //char var[1];
    int numbers[size_array],
...
/*
for(int i=1;i<=size_arr_x;i++)
{
    for(int j=1;j<=size_arr_y;j++)
    {
        arr[i-1][j-1]=i*j;
    }
}
