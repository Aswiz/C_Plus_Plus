...
void hanoi(int n, int start, int finish, int buffer),
         ...
         showResults(int coulumns, int rings);
...
//columns
    ...
    {
        numberForTesting=int(argv[1][i])-48;
        for(int j=1;j<strlen(argv[1])-i;j++)
        {
            numberForTesting*=10;
            // std::cout << "/* numberForTesting = " << numberForTesting << '\n';
        }
        columns+=numberForTesting;
    }
    numberForTesting=0;
//rings
    for(int i=0; i<strlen(argv[2]); i++)
    {
        numberForTesting=int(argv[2][i])-48;
        for(int j=1;j<strlen(argv[2])-i;j++)
        {
            numberForTesting*=10;
            // std::cout << "/* numberForTesting = " << numberForTesting << '\n';
        }
        rings+=numberForTesting;
    }
    numberForTesting=0;
//finish
    for(int i=0; i<strlen(argv[3]); i++)
    {
        numberForTesting=int(argv[3][i])-48;
        for(int j=1;j<strlen(argv[3])-i;j++)
        {
            numberForTesting*=10;
            // std::cout << "/* numberForTesting = " << numberForTesting << '\n';
        }
        finish+=numberForTesting;
    }
    numberForTesting=0;

    // std::cout << "/* columns = " << columns << '\n';
    // std::cout << "/* rings = " << rings << '\n';
...
// Функция показываем количество столбов, колец и шагов
void showResults(int coulumns, int rings)
{
    // std::cout<< '\n' << "coulumns       = " << coulumns << '\n';
    // std::cout << "rings          = " << rings << '\n';
    // std::cout << "numbersOfSteps = " << numbersOfSteps << '\n';
    // std::cout << "   " << coulumns << "         "
    //           << rings << "          "
    //           << numbersOfSteps << '\n';
    std::cout << numbersOfSteps << '\n';
}
